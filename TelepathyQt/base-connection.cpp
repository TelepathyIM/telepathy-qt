...
// Conn.I.Rooms
// The BaseConnectionRoomsInterface code is fully or partially generated by the TelepathyQt-Generator.
struct TP_QT_NO_EXPORT BaseConnectionRoomsInterface::Private {
    Private(BaseConnectionRoomsInterface *parent)
        : connection(0),
          adaptee(new BaseConnectionRoomsInterface::Adaptee(parent))
    {
    }

    QStringList roomAttributeInterfaces;
    GetRoomAttributesCallback getRoomAttributesCB;
    BaseConnection *connection;
    BaseConnectionRoomsInterface::Adaptee *adaptee;
};

BaseConnectionRoomsInterface::Adaptee::Adaptee(BaseConnectionRoomsInterface *interface)
    : QObject(interface),
      mInterface(interface)
{
}
...
QStringList BaseConnectionRoomsInterface::Adaptee::roomAttributeInterfaces() const
{
    return mInterface->roomAttributeInterfaces();
}

void BaseConnectionRoomsInterface::Adaptee::getRoomAttributes(const Tp::UIntList &handles, const QStringList &interfaces,
        const Tp::Service::ConnectionInterfaceRoomsAdaptor::GetRoomAttributesContextPtr &context)
{
    debug() << "BaseConnectionRoomsInterface::Adaptee::getRoomAttributes";
    DBusError error;
    Tp::RoomAttributesMap attributes = mInterface->getRoomAttributes(handles, interfaces, &error);
    if (error.isValid()) {
        context->setFinishedWithError(error.name(), error.message());
        return;
    }
...
void BaseConnectionRoomsInterface::Adaptee::getRoomByID(const QString &identifier, const QStringList &interfaces,
        const Tp::Service::ConnectionInterfaceRoomsAdaptor::GetRoomByIDContextPtr &context)
{
    debug() << "BaseConnectionRoomsInterface::Adaptee::getRoomByID" << identifier;
    DBusError error;
    uint handle;
    QVariantMap attributes;

    mInterface->getRoomByID(identifier, interfaces, handle, attributes, &error);
    if (error.isValid()) {
        context->setFinishedWithError(error.name(), error.message());
        return;
    }
...
/**
 * \class BaseConnectionRoomsInterface
 * \ingroup serviceconn
 * \headerfile TelepathyQt/base-connection.h <TelepathyQt/BaseConnection>
 *
 * \brief Base class for implementations of Connection.Interface.Rooms
 */

/**
 * Class constructor.
 */
BaseConnectionRoomsInterface::BaseConnectionRoomsInterface()
    : AbstractConnectionInterface(TP_QT_IFACE_CONNECTION_INTERFACE_ROOMS),
      mPriv(new Private(this))
{
}
...
void BaseConnectionRoomsInterface::setBaseConnection(BaseConnection *connection)
{
    mPriv->connection = connection;
}
 ...
 * \return The immutable properties of this interface.
 */
QVariantMap BaseConnectionRoomsInterface::immutableProperties() const
{
    QVariantMap map;
    return map;
}

QStringList BaseConnectionRoomsInterface::roomAttributeInterfaces() const
{
    return mPriv->roomAttributeInterfaces;
}

void BaseConnectionRoomsInterface::setRoomAttributeInterfaces(const QStringList &roomAttributeInterfaces)
{
    mPriv->roomAttributeInterfaces = roomAttributeInterfaces;
}

void BaseConnectionRoomsInterface::createAdaptor()
{
    (void) new Tp::Service::ConnectionInterfaceRoomsAdaptor(dbusObject()->dbusConnection(),
            mPriv->adaptee, dbusObject());
}

void BaseConnectionRoomsInterface::setGetRoomAttributesCallback(const GetRoomAttributesCallback &cb)
{
    mPriv->getRoomAttributesCB = cb;
}

Tp::RoomAttributesMap BaseConnectionRoomsInterface::getRoomAttributes(const Tp::UIntList &handles, const QStringList &interfaces, DBusError *error)
{
    if (!mPriv->getRoomAttributesCB.isValid()) {
        error->set(TP_QT_ERROR_NOT_IMPLEMENTED, QLatin1String("Not implemented"));
        return Tp::RoomAttributesMap();
    }
    return mPriv->getRoomAttributesCB(handles, interfaces, error);
}

void BaseConnectionRoomsInterface::getRoomByID(const QString &identifier, const QStringList &interfaces, uint &handle, QVariantMap &attributes, DBusError *error)
{
    const Tp::UIntList handles = mPriv->connection->requestHandles(Tp::HandleTypeRoom, QStringList() << identifier, error);
    if (error->isValid() || handles.isEmpty()) {
        // The check for empty handles is paranoid, because the error must be set in such case.
        error->set(TP_QT_ERROR_INVALID_HANDLE, QLatin1String("Could not process ID"));
        return;
    ...
    const Tp::RoomAttributesMap result = getRoomAttributes(handles, interfaces, error);
