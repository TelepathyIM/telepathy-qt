...
// Conn.I.ChatRead
// The BaseConnectionChatReadInterface code is fully or partially generated by the TelepathyQt-Generator.
struct TP_QT_NO_EXPORT BaseConnectionChatReadInterface::Private {
    Private(BaseConnectionChatReadInterface *parent,
            const Tp::ReadReportingSupportFlags &readReportingSupport)
        : readReportingSupport(readReportingSupport),
          adaptee(new BaseConnectionChatReadInterface::Adaptee(parent))
    {
    }

    Tp::ReadReportingSupportFlags readReportingSupport;
    MarkReadCallback markReadCB;
    MarkUnreadCallback markUnreadCB;
    BaseConnectionChatReadInterface::Adaptee *adaptee;
};

BaseConnectionChatReadInterface::Adaptee::Adaptee(BaseConnectionChatReadInterface *interface)
    : QObject(interface),
      mInterface(interface)
{
}
...
uint BaseConnectionChatReadInterface::Adaptee::readReportingSupport() const
{
    return mInterface->readReportingSupport();
}

void BaseConnectionChatReadInterface::Adaptee::markRead(const QVariantMap &channelRequest, const QString &messageToken,
        const Tp::Service::ConnectionInterfaceChatReadAdaptor::MarkReadContextPtr &context)
{
    debug() << "BaseConnectionChatReadInterface::Adaptee::markRead";
    DBusError error;
    mInterface->markRead(channelRequest, messageToken, &error);
    if (error.isValid()) {
        context->setFinishedWithError(error.name(), error.message());
        return;
    }
...
void BaseConnectionChatReadInterface::Adaptee::markUnread(const QVariantMap &channelRequest,
        const Tp::Service::ConnectionInterfaceChatReadAdaptor::MarkUnreadContextPtr &context)
{
    debug() << "BaseConnectionChatReadInterface::Adaptee::markUnread";
    DBusError error;
    mInterface->markUnread(channelRequest, &error);
    if (error.isValid()) {
        context->setFinishedWithError(error.name(), error.message());
        return;
    }
...
/**
 * \class BaseConnectionChatReadInterface
 * \ingroup serviceconn
 * \headerfile TelepathyQt/base-connection.h <TelepathyQt/BaseConnection>
 *
 * \brief Base class for implementations of Connection.Interface.ChatRead
 */

/**
 * Class constructor.
 */
BaseConnectionChatReadInterface::BaseConnectionChatReadInterface(const Tp::ReadReportingSupportFlags &readReportingSupport)
    : AbstractConnectionInterface(TP_QT_IFACE_CONNECTION_INTERFACE_CHAT_READ),
      mPriv(new Private(this, readReportingSupport))
{
}
 ...
 * Class destructor.
 */
BaseConnectionChatReadInterface::~BaseConnectionChatReadInterface()
{
    delete mPriv;
}
 ...
 * \return The immutable properties of this interface.
 */
QVariantMap BaseConnectionChatReadInterface::immutableProperties() const
{
    QVariantMap map;
    map.insert(TP_QT_IFACE_CONNECTION_INTERFACE_CHAT_READ + QLatin1String(".ReadReportingSupport"),
               QVariant::fromValue(mPriv->adaptee->readReportingSupport()));
    return map;
}

Tp::ReadReportingSupportFlags BaseConnectionChatReadInterface::readReportingSupport() const
{
    return mPriv->readReportingSupport;
}

void BaseConnectionChatReadInterface::createAdaptor()
{
    (void) new Tp::Service::ConnectionInterfaceChatReadAdaptor(dbusObject()->dbusConnection(),
            mPriv->adaptee, dbusObject());
}

void BaseConnectionChatReadInterface::setMarkReadCallback(const MarkReadCallback &cb)
{
    mPriv->markReadCB = cb;
}

void BaseConnectionChatReadInterface::markRead(const QVariantMap &channelRequest, const QString &messageToken, DBusError *error)
{
    if (!mPriv->markReadCB.isValid()) {
        error->set(TP_QT_ERROR_NOT_IMPLEMENTED, QLatin1String("Not implemented"));
        return;
    }
    return mPriv->markReadCB(channelRequest, messageToken, error);
}

void BaseConnectionChatReadInterface::setMarkUnreadCallback(const MarkUnreadCallback &cb)
{
    mPriv->markUnreadCB = cb;
}

void BaseConnectionChatReadInterface::markUnread(const QVariantMap &channelRequest, DBusError *error)
{
    if (!mPriv->markUnreadCB.isValid()) {
        error->set(TP_QT_ERROR_NOT_IMPLEMENTED, QLatin1String("Not implemented"));
        return;
    }
    return mPriv->markUnreadCB(channelRequest, error);
}
