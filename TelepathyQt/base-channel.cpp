...
// Chan.T.ContactSearch
// The BaseChannelContactSearchType code is fully or partially generated by the TelepathyQt-Generator.
struct TP_QT_NO_EXPORT BaseChannelContactSearchType::Private {
    Private(BaseChannelContactSearchType *parent,
            uint limit,
        ...
        : searchState(0),
          ...
          server(server),
          adaptee(new BaseChannelContactSearchType::Adaptee(parent))
    {
    }
    ...
    MoreCallback moreCB;
    StopCallback stopCB;
    BaseChannelContactSearchType::Adaptee *adaptee;
};

BaseChannelContactSearchType::Adaptee::Adaptee(BaseChannelContactSearchType *interface)
    : QObject(interface),
      mInterface(interface)
{
}
...
uint BaseChannelContactSearchType::Adaptee::searchState() const
{
    return mInterface->searchState();
}

uint BaseChannelContactSearchType::Adaptee::limit() const
{
    return mInterface->limit();
}

QStringList BaseChannelContactSearchType::Adaptee::availableSearchKeys() const
{
    return mInterface->availableSearchKeys();
}

QString BaseChannelContactSearchType::Adaptee::server() const
{
    return mInterface->server();
}

void BaseChannelContactSearchType::Adaptee::search(const Tp::ContactSearchMap &terms,
        const Tp::Service::ChannelTypeContactSearchAdaptor::SearchContextPtr &context)
{
    debug() << "BaseChannelContactSearchType::Adaptee::search";
    DBusError error;
    mInterface->search(terms, &error);
    if (error.isValid()) {
        context->setFinishedWithError(error.name(), error.message());
        return;
    }
...
void BaseChannelContactSearchType::Adaptee::more(
        const Tp::Service::ChannelTypeContactSearchAdaptor::MoreContextPtr &context)
{
    debug() << "BaseChannelContactSearchType::Adaptee::more";
    DBusError error;
    mInterface->more(&error);
    if (error.isValid()) {
        context->setFinishedWithError(error.name(), error.message());
        return;
    }
...
void BaseChannelContactSearchType::Adaptee::stop(
        const Tp::Service::ChannelTypeContactSearchAdaptor::StopContextPtr &context)
{
    debug() << "BaseChannelContactSearchType::Adaptee::stop";
    DBusError error;
    mInterface->stop(&error);
    if (error.isValid()) {
        context->setFinishedWithError(error.name(), error.message());
        return;
    }
...
/**
 * \class BaseChannelContactSearchType
 * \ingroup servicechannel
 * \headerfile TelepathyQt/base-channel.h <TelepathyQt/BaseChannel>
 *
 * \brief Base class for implementations of Channel.Type.ContactSearch
 */

/**
 * Class constructor.
 */
BaseChannelContactSearchType::BaseChannelContactSearchType(uint limit,
                                                           const QStringList &availableSearchKeys,
                                                           const QString &server)
    : AbstractChannelInterface(TP_QT_IFACE_CHANNEL_TYPE_CONTACT_SEARCH),
      mPriv(new Private(this, limit, availableSearchKeys, server))
{
}
 ...
 * Class destructor.
 */
BaseChannelContactSearchType::~BaseChannelContactSearchType()
{
    delete mPriv;
}
 ...
 * \return The immutable properties of this interface.
 */
QVariantMap BaseChannelContactSearchType::immutableProperties() const
{
    QVariantMap map;
    map.insert(TP_QT_IFACE_CHANNEL_TYPE_CONTACT_SEARCH + QLatin1String(".Limit"),
               QVariant::fromValue(limit()));
    map.insert(TP_QT_IFACE_CHANNEL_TYPE_CONTACT_SEARCH + QLatin1String(".AvailableSearchKeys"),
               QVariant::fromValue(availableSearchKeys()));
    map.insert(TP_QT_IFACE_CHANNEL_TYPE_CONTACT_SEARCH + QLatin1String(".Server"),
               QVariant::fromValue(server()));
    return map;
}

uint BaseChannelContactSearchType::searchState() const
{
    return mPriv->searchState;
}

void BaseChannelContactSearchType::setSearchState(uint state, const QString &error, const QVariantMap &details)
{
    mPriv->searchState = state;
    QMetaObject::invokeMethod(mPriv->adaptee, "searchStateChanged", Q_ARG(uint, state), Q_ARG(QString, error), Q_ARG(QVariantMap, details)); //Can simply use emit in Qt5
}

uint BaseChannelContactSearchType::limit() const
{
    return mPriv->limit;
}

QStringList BaseChannelContactSearchType::availableSearchKeys() const
{
    return mPriv->availableSearchKeys;
}

QString BaseChannelContactSearchType::server() const
{
    return mPriv->server;
}

void BaseChannelContactSearchType::createAdaptor()
{
    (void) new Tp::Service::ChannelTypeContactSearchAdaptor(dbusObject()->dbusConnection(),
            mPriv->adaptee, dbusObject());
}

void BaseChannelContactSearchType::setSearchCallback(const SearchCallback &cb)
{
    mPriv->searchCB = cb;
}

void BaseChannelContactSearchType::search(const Tp::ContactSearchMap &terms, DBusError *error)
{
    if (!mPriv->searchCB.isValid()) {
        error->set(TP_QT_ERROR_NOT_IMPLEMENTED, QLatin1String("Not implemented"));
        return;
    }
    return mPriv->searchCB(terms, error);
}

void BaseChannelContactSearchType::setMoreCallback(const MoreCallback &cb)
{
    mPriv->moreCB = cb;
}

void BaseChannelContactSearchType::more(DBusError *error)
{
    if (!mPriv->moreCB.isValid()) {
        error->set(TP_QT_ERROR_NOT_IMPLEMENTED, QLatin1String("Not implemented"));
        return;
    }
    return mPriv->moreCB(error);
}

void BaseChannelContactSearchType::setStopCallback(const StopCallback &cb)
{
    mPriv->stopCB = cb;
}

void BaseChannelContactSearchType::stop(DBusError *error)
{
    if (!mPriv->stopCB.isValid()) {
        error->set(TP_QT_ERROR_NOT_IMPLEMENTED, QLatin1String("Not implemented"));
        return;
    }
    return mPriv->stopCB(error);
}

void BaseChannelContactSearchType::searchResultReceived(const Tp::ContactSearchResultMap &result)
{
    QMetaObject::invokeMethod(mPriv->adaptee, "searchResultReceived", Q_ARG(Tp::ContactSearchResultMap, result)); //Can simply use emit in Qt5
}
