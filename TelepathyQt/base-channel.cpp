...
// Chan.I.MessageArchive
// The BaseChannelMessageArchiveInterface code is fully or partially generated by the TelepathyQt-Generator.
struct TP_QT_NO_EXPORT BaseChannelMessageArchiveInterface::Private {
    Private(BaseChannelMessageArchiveInterface *parent,
            const QStringList &supportedFilterKeys)
        : supportedFilterKeys(supportedFilterKeys),
          adaptee(new BaseChannelMessageArchiveInterface::Adaptee(parent))
    {
    }

    QStringList supportedFilterKeys;
    GetMessagesCallback getMessagesCB;
    BaseChannelMessageArchiveInterface::Adaptee *adaptee;
};

BaseChannelMessageArchiveInterface::Adaptee::Adaptee(BaseChannelMessageArchiveInterface *interface)
    : QObject(interface),
      mInterface(interface)
{
}
...
QStringList BaseChannelMessageArchiveInterface::Adaptee::supportedFilterKeys() const
{
    return mInterface->supportedFilterKeys();
}

void BaseChannelMessageArchiveInterface::Adaptee::getMessages(const QVariantMap &filter,
        const Tp::Service::ChannelInterfaceMessageArchiveAdaptor::GetMessagesContextPtr &context)
{
    debug() << "BaseChannelMessageArchiveInterface::Adaptee::getMessages";
    DBusError error;
    mInterface->getMessages(filter, &error);
    if (error.isValid()) {
        context->setFinishedWithError(error.name(), error.message());
        return;
    }
...
/**
 * \class BaseChannelMessageArchiveInterface
 * \ingroup servicechannel
 * \headerfile TelepathyQt/base-channel.h <TelepathyQt/BaseChannel>
 *
 * \brief Base class for implementations of Channel.Interface.MessageArchive
 */

/**
 * Class constructor.
 */
BaseChannelMessageArchiveInterface::BaseChannelMessageArchiveInterface(const QStringList &supportedFilterKeys)
    : AbstractChannelInterface(TP_QT_IFACE_CHANNEL_INTERFACE_MESSAGE_ARCHIVE),
      mPriv(new Private(this, supportedFilterKeys))
{
}
 ...
 * Class destructor.
 */
BaseChannelMessageArchiveInterface::~BaseChannelMessageArchiveInterface()
{
    delete mPriv;
}
 ...
 * \return The immutable properties of this interface.
 */
QVariantMap BaseChannelMessageArchiveInterface::immutableProperties() const
{
    QVariantMap map;
    map.insert(TP_QT_IFACE_CHANNEL_INTERFACE_MESSAGE_ARCHIVE + QLatin1String(".SupportedFilterKeys"),
               QVariant::fromValue(supportedFilterKeys()));
    return map;
}

QStringList BaseChannelMessageArchiveInterface::supportedFilterKeys() const
{
    return mPriv->supportedFilterKeys;
}

void BaseChannelMessageArchiveInterface::createAdaptor()
{
    (void) new Tp::Service::ChannelInterfaceMessageArchiveAdaptor(dbusObject()->dbusConnection(),
            mPriv->adaptee, dbusObject());
}

void BaseChannelMessageArchiveInterface::setGetMessagesCallback(const GetMessagesCallback &cb)
{
    mPriv->getMessagesCB = cb;
}

void BaseChannelMessageArchiveInterface::getMessages(const QVariantMap &filter, DBusError *error)
{
    if (!mPriv->getMessagesCB.isValid()) {
        error->set(TP_QT_ERROR_NOT_IMPLEMENTED, QLatin1String("Not implemented"));
        return;
    }
    return mPriv->getMessagesCB(filter, error);
}
