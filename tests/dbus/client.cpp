...
void TestClient::initTestCase()
{
    ...
    TpDBus::ChannelDetails channelDetails = { QDBusObjectPath(mText1ChanPath), QVariantMap() };
        ...
        const QString &clientBusName, const QString &clientObjectPath)
{
    ...
    TpDBus::ObjectImmutablePropertiesMap reqPropsMap;
    ...
    observeIface->ObserveChannel(QDBusObjectPath(mAccount->objectPath()),
            ...
            QDBusObjectPath(mText1ChanPath), QVariantMap(),
            QDBusObjectPath("/"),
            reqPropsMap,
            QVariantMap());
    QCOMPARE(mLoop->exec(), 0);
...
void TestClient::testAddDispatchOperation()
{
    ...
    dispatchOperationProperties.insert(
            ...
            QVariant::fromValue(TpDBus::ObjectPathList() << QDBusObjectPath(mClientObject1Path)
    ...
    QCOMPARE(client->mAddDispatchOperationChannel, mText1ChanPath);
    ...
    TpDBus::ObjectPathList expectedHandledChannels;
    Q_FOREACH (const TpDBus::ChannelDetails &details, mCDO->Channels()) {
...
void TestClient::testHandleChannels()
{
    ...
    handler2Iface->HandleChannel(QDBusObjectPath(mAccount->objectPath()),
            QDBusObjectPath(mConn->objectPath()),
            QDBusObjectPath(mText2ChanPath), QVariantMap(),
            requestsSatisfied,
            mUserActionTime,
            QVariantMap());
    QCOMPARE(mLoop->exec(), 0);
